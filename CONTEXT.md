...
### Chronicle
Read-only retrieval of historical sessions: search, text display, JSON export, and human-readable HTML generation. Chronicle does not restore sessions back into the original agent's live state.
...
memory; `chronicle` reconstructs normalized entries from persisted events. Timeline
