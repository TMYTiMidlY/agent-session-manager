...
### Retrieval (read-only)
`asmgr`'s read-only retrieval of historical sessions: search, text display, JSON export, and human-readable HTML/Markdown generation. Retrieval never restores sessions back into the original agent's live state.
...
memory; `asmgr` reconstructs normalized entries from persisted events. Timeline
