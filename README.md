...
`chronicle` 有三种安装方式，按摩擦从低到高排列。尚未发布到 npm registry。
...
```bash
# Linux x64（macOS 换成 chronicle-darwin-arm64 或 chronicle-darwin-x64）
curl -fsSL https://github.com/TMYTiMidlY/agent-session-manager/releases/latest/download/chronicle-linux-x64 \
  -o ~/.local/bin/chronicle && chmod +x ~/.local/bin/chronicle
chronicle list --agent all
```

Windows 下载 `chronicle-windows-x64.exe`。每个 Release 附带 `SHA256SUMS.txt` 可校验完整性。

> **一处限制：** 二进制基于 Bun，而 Bun 目前未实现 `node:sqlite`，因此**读取 Copilot 实时 SQLite 库**这一个数据源在二进制里会静默跳过（其余数据源——各家 `*.jsonl`、`--file` 指向的任意文件/目录——都正常）。需要该数据源请改用下面的 Node 安装。

### `npm i -g github:`（需 Node ≥ 22）

无需 registry。安装时 `prepare` 钩子会用 esbuild 把 CLI 及其 workspace 依赖打包成单个自包含文件，因此不需要预先 `pnpm build`：
...
卸载：`npm uninstall -g agent-session-manager`。

### 从源码构建

克隆后安装、构建一次（`pnpm install` 会触发 `prepare` 钩子，顺带打出 `dist/chronicle.mjs`）：
...
装成全局 `chronicle`（软链回本仓库；卸载用 `pnpm --filter @agent-session-manager/cli exec npm unlink -g`）：

```bash
pnpm --filter @agent-session-manager/cli exec npm link
chronicle list --agent all
```

不 link 也可在仓库内直接运行：`node packages/cli/dist/index.js list --agent all`（构建产物），或 `pnpm chronicle search "关键词" --agent all`（CLI 经 tsx 运行的 workspace 脚本）。

自行编译原生二进制（需要 [Bun](https://bun.sh)）：`pnpm run binaries`，四个平台的产物落在 `dist/chronicle-*`。

## 首次运行

1. 按上面任一方式装好 `chronicle`。
2. 列出某个 agent 的会话：

   ```bash
   chronicle list --agent copilot
   ```
...
- ~~**单文件分发。**~~ 已实现：`bun build --compile` 打出四平台原生二进制（见 [`.github/workflows/release.yml`](.github/workflows/release.yml)，push tag 时构建并附加到 Release），以及面向 Node 用户的 `npm i -g github:...` 一行安装（`prepare` 钩子用 esbuild 打包，`workspace:*` 依赖在构建期内联）。详见[安装](#install)。
