#!/usr/bin/env bash
# Copilot 历史增量备份 —— restic → S3-compatible (or any restic backend)
...
# 凭据 + 仓库地址（RESTIC_REPOSITORY / AWS_* / RESTIC_PASSWORD / 可选 RESTIC_BIN / BACKUP_EXCLUDE_REWIND）
set -a; source "$DIR/secrets.env"; set +a

RESTIC="${RESTIC_BIN:-$HOME/.local/bin/restic}"
COPILOT="$HOME/.copilot"          # Copilot CLI 写死的状态目录路径，不参数化

DRYRUN=""
[ "${1:-}" = "--dry-run" ] && DRYRUN="--dry-run"

echo "[$(date '+%F %T')] === Copilot 备份开始 ${DRYRUN:+(dry-run)} ==="

# 1) SQLite 一致性：把 WAL 合进主库并清空，使备份到的 .db 自洽
#    库正被活跃 copilot 进程独占时 checkpoint 是 best-effort，失败不影响主库一致性
if [ -z "$DRYRUN" ]; then
  python3 - "$COPILOT/session-store.db" <<'PY' || true
import sqlite3, sys
try:
...
#    --exclude-file 里只放 glob（SQLite 热文件 / 锁）；锚定到 $COPILOT 下的路径
#    在此处用 --exclude 拼出，避免 exclude.txt 硬编码绝对路径
EXCLUDE_ARGS=( --exclude "$COPILOT/logs" )
if [ "${BACKUP_EXCLUDE_REWIND:-0}" = "1" ]; then
  EXCLUDE_ARGS+=( --exclude "$COPILOT/session-state/*/rewind-snapshots" )
fi

"$RESTIC" backup "$COPILOT" \
  --exclude-file "$DIR/exclude.txt" \
  "${EXCLUDE_ARGS[@]}" \
  --tag copilot --tag "$(hostname)" \
  $DRYRUN

# 3) 保留策略 + prune（dry-run 时跳过）
if [ -z "$DRYRUN" ]; then
  echo "[$(date '+%F %T')] === forget + prune ==="
  "$RESTIC" forget --tag copilot \
